...
# -*- coding: utf-8 -*-
"""
Baseline PaCo (Parametric Contrastive Learning) Training Script
Supports: APTOS2019, MIAS, OralCancer, Fingerprint, OCTA
Based on GPaCo/LT/losses.py implementation.
"""
...
# =========================================================
# PaCo Loss (Adapted from GPaCo/LT/losses.py)
# =========================================================
...
class PaCoLoss(nn.Module):
    ...
    def forward(self, features, labels=None, sup_logits=None):
        ...
        # features: [Batch + K, Dim]
        # labels: [Batch + K]
        # sup_logits: [Batch, NumClasses] (only for current batch)

        if sup_logits is not None:
            batch_size = sup_logits.shape[0]
        else:
            # Fallback (though sup_logits is expected)
            batch_size = features.shape[0] - self.K
        labels = labels.contiguous().view(-1, 1)
        mask = torch.eq(labels[:batch_size], labels.T).float().to(device)

        # compute logits (Contrastive)
        # features[:batch_size] vs ALL features (Batch + Queue)
        anchor_dot_contrast = torch.div(
            torch.matmul(features[:batch_size], features.T),
            self.temperature)

        # add supervised logits
        # Only add to the first 'batch_size' columns? 
        # No, PaCo adds supervised logits as "extra positives" essentially?
        # Actually logic in LT/losses.py:
        # anchor_dot_contrast = torch.cat(( (sup_logits + ... ) / self.supt, anchor_dot_contrast), dim=1)
        # This implies anchor_dot_contrast becomes [Batch, NumClasses + (Batch + K)]
        
        if self.weight is not None:
             sup_part = (sup_logits + torch.log(self.weight + 1e-9)) / self.supt
        else:
             sup_part = sup_logits / self.supt
        ...
        # mask-out self-contrast cases (the diagonals in the [Batch, Batch] block)
        # mask is [Batch, Batch+K] originally? No, mask is [Batch, Batch+K]
        # logic in source: mask = torch.eq(labels[:batch], labels.T) -> [Batch, Batch+K]
        ...
        # add ground truth (for the Supervised Logits part)
        ...
        # No smoothing in standard PaCo? LT code has `smooth` param but defaults to 0.
        
        # mask becomes [Batch, NumClasses + Batch + K]
        # one_hot * beta, mask * alpha
        mask = torch.cat((one_hot_label * self.beta, mask * self.alpha), dim=1)

        # compute log_prob
        # Logits mask needs to handle the NumClasses part too
        # In source: logits_mask = torch.cat((torch.ones(batch, num_classes), gamma * logits_mask), dim=1)
...
class Trainer:
    def __init__(self, args):
        ...
        # Calculate Class Frequency for PaCo balancing
        self.class_freq = torch.zeros(self.num_classes)
        for s in self.train_ds.samples: self.class_freq[s['label']] += 1
        ...
        # In paco_lt.py, it seems to use logits from the model which usually has a classifier head.
        # But for contrastive part it uses MoCo. 
        # Here we use a linear classifier on 'f' (backbone output) for the SupLogits part.
        ...
        self.criterion.cal_weight_for_classes(self.class_freq.cpu().numpy())
    ...
    def train_epoch(self, epoch):
        ...
        for v1, v2, labels in loader:
            ...
            logits = self.classifier(f_q) # Supervised Logits from backbone features
            
            # K
            with torch.no_grad():
                f_k, z_k = self.model_k(v2)
            
            # Prepare features for PaCo Loss: [Batch + Queue]
            ...
            loss = self.criterion(features, labels=all_labels, sup_logits=logits)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=float, default=0.01) # Higher LR for SGD usually
