...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.2, mlp=False, feat_dim=2048, num_classes=1000):
        ...
        if mlp:  # hack: brute-force replacement
            ...
            # GPaCo / MoCo v2 uses 2-layer MLP: Linear -> ReLU -> Linear
            self.encoder_q.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(inplace=True), nn.Linear(dim_mlp, dim))
        ...
        self.head_fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.BatchNorm1d(dim_mlp), nn.ReLU(inplace=True),
    ...
    def _train(self, im_q, im_k, labels):
        ...
        bcl_features = torch.cat((q.unsqueeze(1), k.unsqueeze(1)), dim=1)
        ...
        centers_logits = F.normalize(self.head_fc(self.linear.weight), dim=1)

        return features, target, logits, centers_logits, bcl_features
